...
class WorkflowExecutor:
    ...
    def execute_workflow(self, workflow_steps: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            for i, step in enumerate(workflow_steps):
                ...
                if not step_result["success"]:
                    ...
                    self.execution_metrics["total_variables_created"] += len(
                        step_outputs
                    )
            ...
            if self.progress_callback:
                self.progress_callback({"type": "workflow_error", "error": str(e)})
        ...
        if self.progress_callback:
            self.progress_callback(
                {
                    "type": (
                        "workflow_complete" if results["success"] else "workflow_failed"
                    ),
                    "success": results["success"],
                    "total_duration": results["execution_time"],
                    "completed_steps": results["completed_steps"],
                    "total_steps": total_steps,
                    "variables_created": self.execution_metrics[
    ...
    def execute_step(
        self, step: Dict[str, Any], step_number: int = 1
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if isinstance(output, tuple) and len(output) > 1:
                ...
                human_readable_names = self._get_human_readable_output_names(
                    function_name
                )
                technical_names = self._get_output_names(function_name)

                print(
                    f"Human-readable names for {function_name}: {human_readable_names}"
                )
                ...
                if human_readable_names and len(human_readable_names) == len(output):
                    ...
                    for i, (human_name, val) in enumerate(
                        zip(human_readable_names, output)
                    ):
                        # Use format: AR_Model_2_RMS_Residuals_Feature_Vectors
                        indexed_name = self._make_variable_name(
                            function_name, step_number, human_name
                        )
                        self.variables[indexed_name] = val
                        result["outputs"][indexed_name] = val
                        print(
                            f"Stored human-readable output '{indexed_name}': {type(val)} shape={getattr(val, 'shape', 'no shape')}"
                        )

                        # Also store with technical name for backward compatibility
                        if technical_names and i < len(technical_names):
                            tech_indexed_name = (
                                f"{function_name}_{step_number}_{technical_names[i]}"
                            )
                    ...
                    print(
                        f"Using technical output names (got {len(technical_names)} names for {len(output)} outputs)"
                    )
                    for i, (name, val) in enumerate(zip(technical_names, output)):
                        ...
                        result["outputs"][indexed_name] = val
                        print(
                            f"Stored technical output {indexed_name}: {type(val)} shape={getattr(val, 'shape', 'no shape')}"
                        )
                else:
                    print(
                        f"Using generic output names (got {len(technical_names) if technical_names else 0} technical names for {len(output)} outputs)"
                    )
                    ...
                    for i, val in enumerate(output):
                        ...
                        result["outputs"][indexed_name] = val
                        print(
                            f"Stored generic output {indexed_name}: {type(val)} shape={getattr(val, 'shape', 'no shape')}"
                        )
    ...
    def _prepare_parameters(
        self, func: Callable, parameters: Dict[str, Any], function_name: str = None
    ) -> Dict[str, Any]:
        """
        ...
        for param_name, param_value in parameters.items():
            ...
            if matched_param:
                ...
                if isinstance(param_value, str) and param_value.startswith("Step "):
                    ...
                    try:
                        ...
                        if len(parts) == 2:
                            ...
                            reference_part = parts[
                                1
                            ]  # "function_name" or "function_name_output"
                            ...
                            print(
                                f"    Looking for human-readable matches for '{reference_part}'"
                            )
                                    ...
                                    for var_name in self.variables.keys():
                                        ...
                                        if f"_{step_number}_" in var_name:
                                            ...
                                            if len(var_parts) == 2:
                                                ...
                                                if (
                                                    "rms" in key_phrase.lower()
                                                    and "rms" in var_suffix
                                                    and "residual" in key_phrase.lower()
                                                    and "residual" in var_suffix
                                                ):
                                                    var_key = var_name
                                                    print(
                                                        f"    Found semantic human-readable match: {var_key}"
                                                    )
                                                    break
                            ...
                            if var_key is None:
                                ...
                                print(
                                    f"    Parsing '{reference_part}' with parts: {parts}"
                                )
                                if len(parts) >= 2:
                                    ...
                                    for i in range(1, len(parts)):
                                        ...
                                        test_key = (
                                            f"{func_name}_{step_number}_{output_name}"
                                        )
                            ...
                            if var_key and var_key in self.variables:
                                ...
                                print(
                                    f"  Resolved {param_name} -> {matched_param} = {var_key}"
                                )
                                if hasattr(var_value, "shape"):
                                    print(f"    Variable shape: {var_value.shape}")
                            else:
                                print(
                                    f"  Warning: Variable not found with key {var_key}: {list(self.variables.keys())}"
                                )
                                ...
                                for i in range(1, len(parts) + 1):
                                    ...
                                    if any(
                                        var.startswith(
                                            f"{potential_func}_{step_number}_"
                                        )
                                        for var in self.variables.keys()
                                    ):
                                        func_name = potential_func
                                        output_hint = (
                                            "_".join(parts[i:]).lower()
                                            if i < len(parts)
                                            else ""
                                        )
                                        break
                                ...
                                print(
                                    f"    Semantic analysis - func: '{func_name}', hint: '{output_hint}'"
                                )
                                if any(
                                    hint in output_hint for hint in ["rms", "residual"]
                                ):
                                    # RMS/residuals are often output 1 in multi-output functions
                                    print(f"    Detected RMS/residual pattern")
                                    fallback_keys.append(
                                        f"{func_name}_{step_number}_output_1"
                                    )
                                    fallback_keys.append(
                                        f"{func_name}_{step_number}_output_0"
                                    )
                                elif any(
                                    hint in output_hint
                                    for hint in ["parameters", "params", "coeff"]
                                ):
                                    ...
                                    fallback_keys.append(
                                        f"{func_name}_{step_number}_output_2"
                                    )
                                elif any(
                                    hint in output_hint for hint in ["model", "trained"]
                                ):
                                    ...
                                    fallback_keys.append(
                                        f"{func_name}_{step_number}_out"
                                    )
                                else:
                                    # Try all outputs if no semantic hint
                                    for i in range(5):
                                        fallback_keys.append(
                                            f"{func_name}_{step_number}_output_{i}"
                                        )
                                    fallback_keys.append(
                                        f"{func_name}_{step_number}_out"
                                    )
                                ...
                                for alt_key in fallback_keys:
                                    if alt_key in self.variables:
                                        prepared[matched_param] = self.variables[
                                            alt_key
                                        ]
                                        print(f"  Found semantic fallback: {alt_key}")
                                        if hasattr(self.variables[alt_key], "shape"):
                                            print(
                                                f"    Fallback shape: {self.variables[alt_key].shape}"
                                            )
                                ...
                                if not found_alternative:
                                    print(
                                        f"  No semantic fallback found for '{output_hint}'"
                                    )
                        else:
                ...
                print(
                    f"  Warning: Parameter '{param_name}' not found in function signature"
                )
    ...
    def list_variables(self) -> Dict[str, str]:
        ...
        """
        return {
            name: str(type(value).__name__) for name, value in self.variables.items()
        }
    ...
    def _get_human_readable_output_names(
    ...
    def _get_human_readable_input_names(
        self, function_name: str
    ) -> Optional[List[str]]:
        """
    ...
    def _get_function_display_name(self, function_name: str) -> str:
        ...
        if func:
            try:
                ...
                print(
                    f"Warning: Could not parse function display name for {function_name}: {e}"
                )
    ...
    def _make_variable_name(
        self, function_name: str, step_number: int, human_readable_name: str
    ) -> str:
        """
    ...
    def _get_output_descriptions(self, function_name: str) -> Dict[str, str]:
        ...
        if shm_func:
            try:
                ...
                if metadata and metadata.returns:
                    return {
                        return_spec.name: return_spec.description
                        for return_spec in metadata.returns
                    }
            except Exception:
                pass
