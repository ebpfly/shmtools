...
def create_pca_example_session():
    ...
    session.update_step_parameter(
        2, "data", "", "variable_reference", "Step 1: load_3story_data"
    )
    ...
    session.update_step_parameter(
        3, "X", "", "variable_reference", "Step 2: ar_model_rms_residuals_fv"
    )
    ...
    session.update_step_parameter(
        4, "Y", "", "variable_reference", "Step 2: ar_model_rms_residuals_fv"
    )
    session.update_step_parameter(
        4, "model", "", "variable_reference", "Step 3: learn_pca"
    )
...
def create_comparison_session():
    ...
    session.add_function_step(
        "learn_mahalanobis", "Classification - Parametric Detectors"
    )
    session.update_step_parameter(
        5, "X", "", "variable_reference", "Step 2: ar_model_rms_residuals_fv"
    )

    session.add_function_step(
        "score_mahalanobis", "Classification - Parametric Detectors"
    )
    session.update_step_parameter(
        6, "Y", "", "variable_reference", "Step 2: ar_model_rms_residuals_fv"
    )
    session.update_step_parameter(
        6, "model", "", "variable_reference", "Step 5: learn_mahalanobis"
    )

    # Steps 7-8: SVD method
    session.add_function_step("learn_svd", "Classification - Parametric Detectors")
    session.update_step_parameter(
        7, "X", "", "variable_reference", "Step 2: ar_model_rms_residuals_fv"
    )

    session.add_function_step("score_svd", "Classification - Parametric Detectors")
    session.update_step_parameter(
        8, "Y", "", "variable_reference", "Step 2: ar_model_rms_residuals_fv"
    )
    session.update_step_parameter(
        8, "model", "", "variable_reference", "Step 7: learn_svd"
    )
