...
class WorkflowSession:
    ...
    def add_function_step(self, function_name: str, category: str = "",
                         description: str = "", parameters: Optional[List[ParameterSchema]] = None,
                         display_name: Optional[str] = None) -> StepSchema:
        """
        ...
        for step in self.schema.steps:
            if step.step_number > step_number:
                for param in step.parameters:
                    if (param.parameter_type == 'variable_reference' and 
                        param.source_step and param.source_step.startswith(f"Step {step_number}:")):
                        # This parameter references the step being removed
                        param.parameter_type = 'user'
                        param.source_step = None
                        param.value = ""  # Reset to empty, user will need to reconfigure
        
        self.schema.remove_step(step_number)
        self.mark_modified()
    
    def update_step_parameter(self, step_number: int, param_name: str, 
                            value: Any, param_type: str = 'user', 
        ...
        try:
            ...
            try:
                ...
                if function_name == 'load_3story_data':
                    ...
                    from shmtools.classification.outlier_detection import learn_mahalanobis
                    func_obj = learn_mahalanobis
                elif function_name == 'score_mahalanobis':
                    from shmtools.classification.outlier_detection import score_mahalanobis
...
class SessionManager:
    ...
    def create_new_session(self, name: str = "New Workflow", 
                          description: str = "", author: str = "") -> WorkflowSession:
        """
