...
class VariableWorkspacePanel:
    ...
    def _create_panel(self):
        ...
        panel_content = column(
            title, controls, workspace_tabs, min_width=600, sizing_mode="stretch_both"
        )
    ...
    def _create_variables_tab(self):
        ...
        self.variables_table.source.selected.on_change(
            "indices", self._on_variable_select
        )

        variables_content = column(
            self.variables_table, details_title, self.variable_details
        )
    ...
    def _create_connections_tab(self):
        ...
        compact_view_btn.on_click(
            lambda: self._update_connections_display(compact=True)
        )
        expand_view_btn.on_click(
            lambda: self._update_connections_display(compact=False)
        )

        connection_controls = row(
            self.connection_filter, show_arrows_btn, compact_view_btn, expand_view_btn
        )

        connections_content = column(
            connections_title, connection_controls, self.connections_display
        )
    ...
    def _create_tags_tab(self):
        ...
        # Tag controls
        tag_variable = Select(
            title="Variable:", options=[("", "Select Variable")], width=200
        )

        tag_input = TextInput(title="Add Tag:", placeholder="Enter tag name", width=200)
        ...
        quick_tags = row(
            feature_tag_btn, model_tag_btn, scores_tag_btn, intermediate_tag_btn
        )
        ...
        tags_content = column(
            tags_title, tag_controls, predefined_title, quick_tags, self.tags_display
        )
    ...
    def _refresh_variables(self):
        ...
        # Update tag selector
        var_options = [("", "Select Variable")] + [
            (name, name) for name in data["name"]
        ]
    ...
    def _update_tags_display(self):
        ...
        for var_name, tags in self.variable_tags.items():
            tags_str = ", ".join(
                f"<span style='background-color: #e6f3ff; padding: 2px 6px; border-radius: 3px;'>{tag}</span>"
                for tag in tags
            )
    ...
    def _analyze_variable_connections(self) -> Dict[str, Any]:
        ...
        connections = {"steps": {}, "flows": [], "variables": {}}
            ...
            if i < len(step_numbers) - 1:  # Not the last step
                ...
                for next_step in next_steps:
                    for output_var in step_outputs:
                        ...
                        if self._could_be_input_to_step(
                            output_var, next_step, connections
                        ):
                            ...
                            connections["variables"][output_var]["used_by"].append(
                                next_step
                            )
    ...
    def _parse_step_info(self, source: str) -> Optional[Dict[str, Any]]:
        ...
        if source.startswith("Step "):
            try:
                ...
                return {"step_number": step_number, "function_name": func_part}
            except (ValueError, IndexError):
                pass
        return None

    def _could_be_input_to_step(
        self, var_name: str, step_num: int, connections: Dict
    ) -> bool:
        ...
        if any(
            ...
            for pattern in ["data", "features", "signal", "time_series"]
        ):
            if any(
                pattern in func_name
                for pattern in ["process", "analyze", "transform", "learn", "score"]
            ):
                return True

        # Model patterns
        if "model" in var_name_lower and any(
            pattern in func_name for pattern in ["score", "predict", "test", "apply"]
        ):
            return True

        # Output patterns (features, parameters, etc.)
        if any(
            suffix in var_name_lower
            for suffix in ["_fv", "_features", "_params", "_parameters"]
        ):
            if any(
                pattern in func_name
                for pattern in ["learn", "score", "classify", "detect"]
            ):
                return True
        ...
        for step_num in step_numbers:
            ...
            if step_info["outputs"]:
                ...
                for output_var in step_info["outputs"]:
                    ...
                    used_text = (
                        f" → Used by steps: {', '.join(map(str, used_by))}"
                        if used_by
                        else " (unused)"
                    )
                    ...
                    html_parts.append(
                        f"<li><code>{output_var}</code>{tags_text}{used_text}</li>"
                    )
