...
class ParameterControlsPanel:
    ...
    def __init__(self):
        ...
        self.current_function_obj = (
            None  # Store actual function object for introspection
        )
    ...
    def _create_panel(self):
        ...
        # Function info
        function_info = Div(
            text="<p>Select a workflow step to configure parameters.</p>", width=280
        )
        ...
        # Apply/Reset buttons
        apply_btn = Button(
            label="Apply Changes", button_type="primary", width=130, disabled=True
        )
        reset_btn = Button(label="Reset to Defaults", width=130, disabled=True)
    ...
    def get_available_outputs(self, before_step: int) -> List[str]:
        ...
        for i, step in enumerate(self.workflow_steps):
            ...
            human_readable_names = executor._get_human_readable_output_names(
                function_name
            )
            ...
            if human_readable_names:
                ...
                for human_name in human_readable_names:
                    display_text = (
                        f"Step {step_number}: {function_display_name} → {human_name}"
                    )
    ...
    def _parameter_to_readable(self, param_name: str) -> str:
        ...
        for i, word in enumerate(words):
            ...
            if lower_word in abbrev_map:
                ...
                if i == 0 or (
                    i > 0 and words[i - 1].lower() not in ["is", "has", "with", "use"]
                ):
    ...
    def _create_parameter_widget_from_signature(
        self, param_name: str, param_info, current_value, step_index: int
    ):
        ...
        if (
            hasattr(self, "human_readable_param_names")
            and param_name in self.human_readable_param_names
        ):
        ...
        if current_value is not None and not (
            isinstance(current_value, str) and current_value.startswith("Step ")
        ):
    ...
    def set_function(
        self, function_name: str, parameters: Dict[str, Any], step_index: int = 0
    ):
        """
            ...
            try:
                ...
                if metadata:
                    function_description = (
                        metadata.brief_description or function_description
                    )
                    ...
                    if metadata.verbose_call:
                        ...
                        if self.current_function_obj:
                            ...
                            for i, (param_name, human_name) in enumerate(
                                zip(param_list, input_names)
                            ):
        ...
        self.function_info.text = (
            f"<h4>{display_name}</h4><p>{function_description}</p>"
        )
        ...
        if self.current_function_obj:
            ...
            try:
                ...
                for param_name, param_info in sig.parameters.items():
                    ...
                    if widget_wrapper:
                        widgets.append(
                            widget_wrapper.container
                        )  # Add container to display
                        self.parameter_widgets[param_name] = (
                            widget_wrapper  # Store wrapper for value extraction
                        )
                ...
                error_msg = Div(
                    text=f"<p style='color: red;'>Error inspecting function: {e}</p>"
                )
                self.params_container.children = [error_msg]
        else:
            error_msg = Div(
                text=f"<p style='color: red;'>Function {function_name} not found</p>"
            )
            self.params_container.children = [error_msg]
    ...
    def _create_parameter_widget(
        self, param_name: str, spec: Dict[str, Any], current_value: Any
    ):
        """
        ...
        default_value = (
            current_value if current_value is not None else spec.get("default")
        )
        ...
        if param_type == "float":
            ...
            return CheckboxGroup(
                labels=[title], active=[0] if default_value else [], width=250
            )
        else:
            return TextInput(
                title=title,
    ...
    def clear_selection(self):
        ...
        # Reset UI
        self.function_info.text = (
            "<p>Select a workflow step to configure parameters.</p>"
        )
        self.params_container.children = []
    ...
    def get_current_parameters(self) -> Dict[str, Any]:
        ...
        for param_name, widget_wrapper in self.parameter_widgets.items():
            ...
            if hasattr(widget_wrapper, "output_select") and hasattr(
                widget_wrapper, "value_widget"
            ):
                ...
                if output_selection != "[Select Input]":
                    ...
                    if hasattr(widget_wrapper.value_widget, "value"):
                        ...
                        parameters[param_name] = (
                            len(widget_wrapper.value_widget.active) > 0
                        )
