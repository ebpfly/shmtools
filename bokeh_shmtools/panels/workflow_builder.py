...
from bokeh.models import (
    ...
    ColumnDataSource, TextInput, Select, FileInput
)
...
class WorkflowBuilderPanel:
    ...
    def _create_panel(self):
        ...
        session_buttons = row(new_session_btn, save_session_btn, save_as_btn, file_input)
        ...
        progress_bar = Div(
            text=self._generate_progress_html(0),
            width=350, 
            height=30,
            visible=False
        )
    ...
    def add_function(self, function_name: str, display_name: str = None):
        ...
        step = {
            ...
            "display_name": display_name or function_name,  # Human-readable name for display
            "status": "Ready",
            "parameters": {},
            "outputs": None
        }
    ...
    def _update_steps_table(self):
        ...
        data = {
            ...
            "function": [step.get("display_name", step["function"]) for step in self.workflow_steps],  # Use display name
    ...
    def _run_workflow(self):
        ...
        print(f"Steps: {[step.get('display_name', step['function']) for step in self.workflow_steps]}")
    ...
    def _on_progress_update(self, progress_info: Dict[str, Any]):
        ...
        if progress_type == "workflow_start":
            ...
            if step_num <= len(self.workflow_steps):
                display_name = self.workflow_steps[step_num - 1].get("display_name", function_name)
            ...
            self.status_display.text = f"❌ Step {step_num}: Failed after {duration:.2f}s - {error}"
            ...
            self.status_display.text = f"🎉 Workflow completed successfully in {total_duration:.2f}s"
        ...
        try:
            ...
            for step in self.current_session.steps:
                workflow_step = {
                    ...
                    "display_name": getattr(step, 'display_name', step.function_name),  # Use display name from session
