...
from bokeh.models import (
    Div, Tabs, TabPanel, Button, Select, Column, Row,
...
class ResultsViewerPanel:
    ...
    def _create_export_tab(self):
        ...
        export_content = column(
            ...
            Div(text="<p>Workflow completed successfully. Results available above.</p>")
        )
    ...
    def update_results(self, results):
        ...
        if self.workflow_results:
            ...
            self.plot_select.value = list(self.workflow_results.keys())[-1]  # Select latest result
    ...
    def _auto_visualize(self, output_name: str, output_value):
        ...
        if hasattr(output_value, 'shape'):
            ...
            if "ar_model" in output_name.lower() and len(output_value) >= 3:
                ...
                if hasattr(output_value[2], 'shape') and len(output_value[2].shape) == 3:
                ...
                if hasattr(output_value[0], 'shape') and hasattr(output_value[1], 'shape'):
                    ...
                    if output_value[0].ndim == 1 and output_value[1].ndim == 1:
                        ...
                        if "psd" in output_name.lower() or "freq" in output_name.lower():
                            self._plot_frequency_response(output_name, output_value[0], output_value[1])
                        else:
                            # AR model output or similar - plot first array as coefficients
                            self._plot_1d_array(f"{output_name} (coefficients)", output_value[0])
                    else:
                        # Multi-dimensional - visualize first element
                        self._auto_visualize(f"{output_name}[0]", output_value[0])
                elif hasattr(output_value[0], 'shape'):
                    # First element is array - visualize it
                    self._auto_visualize(f"{output_name} (primary output)", output_value[0])
            else:
                # Multiple outputs - show summary
                self._display_text_summary(output_name, f"Tuple/List with {len(output_value)} elements")
    ...
    def _plot_outlier_scores(self):
        ...
        self.plot.line([0, 100], [3, 3], line_width=2, color="red", 
                      line_dash="dashed", legend_label="Threshold")
    ...
    def _plot_multi_channel_time_series(self, name: str, data: np.ndarray):
        ...
        for ch in range(min(data.shape[1], 5)):  # Limit to 5 channels
            self.plot.line(time_points, data[:, ch], 
                          line_width=2, color=colors[ch % len(colors)], 
                          alpha=0.8, legend_label=f"Ch {ch+1}")
    ...
    def _plot_frequency_response(self, name: str, freqs: np.ndarray, response: np.ndarray):
