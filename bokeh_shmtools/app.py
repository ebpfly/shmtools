...
def create_app():
    ...
    def add_function_to_workflow():
        try:
            if (
                hasattr(function_panel, "selected_function")
                and function_panel.selected_function
            ):
                # Get both technical and display names
                technical_name = function_panel.selected_function
                display_name = getattr(
                    function_panel, "selected_display_name", technical_name
                )
        ...
        try:
            if new and len(new) > 0:
                ...
                if step_idx < len(workflow_panel.workflow_steps):
                    ...
                    params_panel.set_function(
                        step["function"], step["parameters"], step_idx
                    )
    ...
    def update_results_viewer():
        ...
        if hasattr(workflow_panel, "executor") and workflow_panel.executor.variables:
            ...
            for i, step in enumerate(workflow_panel.workflow_steps):
                if step.get("outputs"):
                    step_result = {"success": True, "outputs": step["outputs"]}
    ...
    def on_apply_parameters():
        try:
            if (
                hasattr(params_panel, "current_function")
                and params_panel.current_function
                and hasattr(workflow_panel, "selected_step_idx")
                and workflow_panel.selected_step_idx is not None
            ):
