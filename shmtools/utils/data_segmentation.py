...
def segment_time_series(
    ...
    if data.ndim != 3:
        raise ValueError(f"Data must be 3D (TIME, CHANNELS, INSTANCES), got shape {data.shape}")
        
    time_points, n_channels, n_instances = data.shape
    
    if segment_length > time_points:
        raise ValueError(f"Segment length ({segment_length}) cannot exceed time series length ({time_points})")
...
def prepare_train_test_split(
    ...
    test_labels = np.concatenate([
        np.zeros(n_test_undamaged, dtype=int),
        np.ones(n_test_damaged, dtype=int)
    ])
