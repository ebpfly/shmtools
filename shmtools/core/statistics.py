...
def fm0_shm(X: np.ndarray, fund_mesh_freq: float, track_orders: Optional[np.ndarray] = None, 
            nfft: int = 512, n_bin_search: int = 2) -> np.ndarray:
    """
    ...
    for i_channel in range(n_channel):
        for i_instance in range(n_instance):
            ...
            sum_amp = _sum_amplitudes(psd, f_psd, fund_mesh_freq, track_orders, n_bin_search)
...
def _sum_amplitudes(psd: np.ndarray, f_psd: np.ndarray, fund_mesh_freq: float,
                   track_orders: np.ndarray, n_bin_search: int) -> float:
    """
...
def fm4_shm(D: np.ndarray) -> np.ndarray:
    ...
    for i_channel in range(n_channel):
        for i_instance in range(n_instance):
            ...
            fm4[i_instance, i_channel] = n_signal * fourth_moment / second_moment_squared
