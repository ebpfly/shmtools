...
def stat_moments_shm(X: np.ndarray) -> np.ndarray:
    ...
    for i in range(t):
        ...
        statistics_fv[i, :] = np.concatenate([mean_vals, std_vals, skew_vals, kurt_vals])
...
def m6a_shm(D: np.ndarray) -> np.ndarray:
    ...
    for i_channel in range(n_channel):
        for i_instance in range(n_instance):
            ...
            if second_moment > 0:
                m6a[i_instance, i_channel] = (n_signal**2 * sixth_moment) / (second_moment**3)
            else:
...
def m8a_shm(D: np.ndarray) -> np.ndarray:
    ...
    for i_channel in range(n_channel):
        for i_instance in range(n_instance):
            ...
            if second_moment > 0:
                m8a[i_instance, i_channel] = (n_signal**3 * eighth_moment) / (second_moment**4)
            else:
...
def compute_damage_features_shm(
    ...
    """
    if features is None:
        features = ['peak_factor', 'impulse_factor', 'clearance_factor', 'shape_factor', 'fm0', 'fm4', 'crest_factor', 'rms', 'stat_moments', 'm6a', 'm8a']
    ...
    feature_functions = {
        ...
        'm6a': m6a_shm,
        'm8a': m8a_shm,
        'fm0': lambda x, axis=axis: fm0_shm(x, fund_mesh_freq=1.0, track_orders=[1], n_fft=None, n_bin_search=3),
        'fm4': fm4_shm
    }
...
def na4m_shm(R: np.ndarray, m2: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if m2 is None:
        ...
        variance_per_instance = np.sum(centered_R**2, axis=0) / n_signal  # Shape: (n_channel, n_instance)
        m2 = np.sum(variance_per_instance, axis=1, keepdims=True) / n_instance  # Shape: (n_channel, 1)
    ...
    for i_channel in range(n_channel):
        for i_instance in range(n_instance):
            ...
            na4m[i_instance, i_channel] = (fourth_moment / n_signal) / (m2[0, i_channel]**2)
...
def nb4m_shm(X: np.ndarray, m2: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if m2 is None:
        ...
        variance_per_instance = np.sum(centered_E**2, axis=0) / n_signal  # Shape: (n_channel, n_instance)
        m2 = np.sum(variance_per_instance, axis=1, keepdims=True) / n_instance  # Shape: (n_channel, 1)
        m2 = m2.T  # Shape: (1, n_channel) to match MATLAB
    ...
    for i_channel in range(n_channel):
        for i_instance in range(n_instance):
            ...
            nb4m[i_instance, i_channel] = (fourth_moment / n_signal) / (m2[0, i_channel]**2)
