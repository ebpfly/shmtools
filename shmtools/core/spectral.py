...
def psd_welch_shm(
    ...
    .. meta::
        :category: Feature Extraction - Spectral
...
def wavelet_shm(wave_type: str, wave_param: np.ndarray) -> np.ndarray:
    ...
    if wave_type == "MORLET":
        ...
        fb = 4 / n_wavelet
        wavelet = (
            np.sqrt(fb) * np.exp(2 * np.pi * 1j * fc * x) * (np.sinc(fb * x / M)) ** M
        )

    else:
        raise ValueError(
            f"Unknown wavelet type: {wave_type}. Must be 'morlet', 'shannon', or 'bspline'"
        )
...
def _dwvd_single(
    ...
    Z = np.concatenate([np.zeros(buffer_length), Z, np.zeros(buffer_length)])
...
def _lpc_spec_single(
    ...
    if not (1 <= model_order < n_win):
        raise ValueError(
            f"Inappropriate Model Order. Consider using model_order <= {rec_max_order} for window length n_win: {n_win}"
        )

    if model_order > rec_max_order:
        print(
            f"WARNING: (model_order:n_win) > (2:3). Consider using model_order <= {rec_max_order} for window length n_win: {n_win}"
        )
