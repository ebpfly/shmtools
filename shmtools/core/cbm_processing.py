...
def _crop_signal(
    X: np.ndarray, tach: np.ndarray, iCross: np.ndarray
...
def _interp_shaft_angle(
    iT: np.ndarray, nSignal: int, gearRatio: float
    ...
    f = interpolate.interp1d(
        iT, phi, kind="cubic", bounds_error=False, fill_value="extrapolate"
    )
    phiS = f(n) * gearRatio
...
def _upsample_signal(
    phiS: np.ndarray, X: np.ndarray, minSPR: float, samplesPerRev: int
    ...
    # Use scipy interpolation
    f_phi = interpolate.interp1d(
        n, phiS, kind="cubic", bounds_error=False, fill_value="extrapolate"
    )
    f_x = interpolate.interp1d(
        n, X, kind="cubic", bounds_error=False, fill_value="extrapolate"
    )
...
def _angular_resample_signal(
    phiS: np.ndarray, X: np.ndarray, samplesPerRev: int, nT: float
) -> Tuple[np.ndarray, np.ndarray]:
    """Angular resample signal to M x desired SPR."""
    M = 2  # Upsample ratio to be decimated later
    phiA = np.linspace(
        0, (nT - 1) * (2 * np.pi), int((M * samplesPerRev) * (nT - 1) + 1)
    )

    # Use scipy interpolation
    f = interpolate.interp1d(
        phiS, X, kind="cubic", bounds_error=False, fill_value="extrapolate"
    )
    xARS = f(phiA)
...
def _downsample_to_spr(
    phiA: np.ndarray, xARS: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    """Downsample to desired SPR."""
    M = 2  # Decimate
    phiA_down = phiA[::M]
