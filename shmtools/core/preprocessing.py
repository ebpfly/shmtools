...
def filter_shm(X: np.ndarray, filter_coef: np.ndarray) -> np.ndarray:
    ...
    filter_coef_expanded = np.tile(
        filter_coef.reshape(-1, 1, 1), (1, n_channel, n_instance)
    )
...
def window_shm(
    win_type: str, win_param: Union[int, list], win_form: Optional[str] = None
) -> np.ndarray:
    """
    ...
    if win_type_upper == "RECTWIN":
        ...
        raise ValueError(
            f"Window type '{win_type}' not supported. "
