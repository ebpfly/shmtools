...
def ars_tach_shm(
    ...
    if len(pulse_indices) < 2:
        raise ValueError(f"Insufficient tachometer pulses detected: {len(pulse_indices)}")
    ...
    if len(instantaneous_period) > 2:
        ...
        if np.sum(valid_mask) < len(instantaneous_period) * 0.5:
            warnings.warn("High number of outlier periods detected in tachometer signal")
    ...
    dt = 1.0 / fs
    instantaneous_freq = 1.0 / (period_interp + 1e-12)  # Add small value to avoid division by zero
    cumulative_angle = np.cumsum(instantaneous_freq * dt * 360.0 * k)  # Convert to degrees
...
def fir1_shm(n: int, Wn: Union[float, Tuple[float, float]], 
    ...
    if isinstance(Wn_norm, (list, tuple)):
        ...
        if Wn_norm <= 0 or Wn_norm >= 1:
            raise ValueError("Critical frequency must be between 0 and Nyquist frequency")
    ...
    if ftype not in filter_types:
        raise ValueError(f"Invalid filter type. Must be one of: {list(filter_types.keys())}")
