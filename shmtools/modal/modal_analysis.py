...
def frf_shm(
    ...
    if block_size > n_time:
        raise ValueError(f"Block size ({block_size}) cannot exceed data length ({n_time})")
...
def rpfit_shm(
    ...
    # Validate inputs
    if frf_data.ndim != 3:
        raise ValueError("FRF data must be 3D array with shape (FREQUENCY, DOFS, INSTANCES)")
    ...
    for instance in range(n_instances):
        ...
        for range_idx, (freq_range, n_modes_range) in enumerate(zip(freq_ranges, n_modes)):
...
def _rpfit_instance(
    ...
    try:
        ...
        if len(modal_poles) < n_modes:
            warnings.warn(f"Only found {len(modal_poles)} modal poles, expected {n_modes}")
            # Pad with zeros if necessary
            modal_poles = np.concatenate([
                modal_poles, 
                np.zeros(n_modes - len(modal_poles), dtype=complex)
            ])
        ...
        for mode_idx, pole in enumerate(modal_poles):
            if pole != 0:
                ...
                residues_array[:, mode_idx] = residue_mag * np.ones(n_dofs, dtype=complex)
