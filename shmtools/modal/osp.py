...
def add_resp_2_geom_shm(
    geom_layout: np.ndarray, resp_xyz: np.ndarray, scale: Optional[float] = None
...
def osp_fisher_info_eiv_shm(
    num_sensors: int, mode_shapes: np.ndarray, cov_matrix: Optional[np.ndarray] = None
    ...
    if num_sensors > n_dof:
        raise ValueError(
            f"Number of sensors ({num_sensors}) cannot exceed DOFs ({n_dof})"
        )
...
def get_sensor_layout_shm(
    op_list: np.ndarray, resp_dof: np.ndarray, node_layout: np.ndarray
) -> np.ndarray:
    """
...
def osp_max_norm_shm(
    ...
    if len(weights) != n_modes:
        raise ValueError(
            f"Number of weights ({len(weights)}) must match modes ({n_modes})"
        )
    ...
    while len(selected_dofs) < num_sensors and available_dofs:
        ...
        sorted_indices = sorted(
            available_dofs, key=lambda x: weighted_norm[x], reverse=True
        )
        ...
        if not selected and available_dofs:
            ...
            warnings.warn(
                f"Relaxed separation constraint for sensor {len(selected_dofs)}"
            )
