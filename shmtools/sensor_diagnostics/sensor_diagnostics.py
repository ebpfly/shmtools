...
def sd_autoclassify_shm(
    ...
    if n_sensors > 2:
        ...
        for m in range(n_sensors - 1, n_sensors // 2 - 1, -1):
            ...
            y = (A_sorted[-1, 2] / (n_sensors - 1)) * m - (A_sorted[-1, 2] / (n_sensors - 1))
            diff = y - A_sorted[m, 2]
    ...
    if cutoff_pos < n_sensors:
        ...
        for i in range(cutoff_pos):
            ...
            if cap_value > ave_healthy and (cap_value - ave_healthy) / ave_healthy > threshold:
                sensor_status[sensor_idx, 1] = 1
                debonded_list.append(sensor_idx + 1)
            # Broken: capacitance lower than average  
            elif cap_value < ave_healthy and (ave_healthy - cap_value) / ave_healthy > threshold:
...
def sd_plot_shm(data_for_plotting: Dict[str, Any]) -> None:
    ...
    for i in range(n_sensors):
        ...
        ax1.scatter(i + 1, A_sorted[n_sensors - 1 - i, 2], 
                   c=[colors[i]], s=50, label=f'Sensor {sensor_id}')
    ...
    if pos < n_sensors:
        ...
        ax1.annotate('', xy=(n_sensors - pos, ax1.get_ylim()[0]),
                    xytext=(1, ax1.get_ylim()[0]),
                    arrowprops=dict(arrowstyle='<-', color='red', lw=2))
        ax1.annotate('', xy=(n_sensors - pos + 1, ax1.get_ylim()[0]),
                    xytext=(n_sensors, ax1.get_ylim()[0]),
                    arrowprops=dict(arrowstyle='<-', color='green', lw=2))
        
        ax1.text(n_sensors // 4, ax1.get_ylim()[0] * 0.1, 'Faulty', 
                color='red', ha='center', fontsize=12)
        ax1.text(3 * n_sensors // 4, ax1.get_ylim()[0] * 0.1, 'Healthy',
                color='green', ha='center', fontsize=12)
    ...
    xticks = [''] + [str(int(A_sorted[n_sensors - i - 1, 3])) for i in range(n_sensors)] + ['']
    ...
    # Create legend
    legend_elements = [plt.Rectangle((0, 0), 1, 1, color='blue', label='Healthy sensors')]
    if broken_list:
        legend_elements.append(plt.Rectangle((0, 0), 1, 1, color='red', label='Broken sensors'))
    if debonded_list:
        legend_elements.append(plt.Rectangle((0, 0), 1, 1, color='magenta', label='De-bonded sensors'))
