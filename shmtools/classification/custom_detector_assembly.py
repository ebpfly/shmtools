...
def assemble_outlier_detector_shm(
    ...
    if not detector_registry.validate_detector_availability(
        detector_type, detector_name
    ):
        available = list(detector_registry.get_detectors_by_type(detector_type).keys())
        raise ValueError(
            f"Detector {detector_name} not available. Available: {available}"
        )
...
def _prompt_parameters(
    detector_type: str, detector_name: str, learn_func_name: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, param_name in enumerate(param_names, 1):
            ...
            default_str = (
                f" (default: {param.default})" if param.default != param.empty else ""
            )
...
def _create_training_function(
    ...
    def assembled_training_function(
        features: np.ndarray,
        k: Optional[int] = None,
        ...
        if detector_type == "nonparametric" and "kernel_function" in learn_params:
            ...
            if isinstance(kernel_name, str):
                ...
                if hasattr(nonparametric, kernel_func_name):
                    learn_params["kernel_fun"] = getattr(
                        nonparametric, kernel_func_name
                    )
                ...
                if isinstance(partition_alg, str):
                    if partition_alg.lower() == "kmeans":
                        ...
                        if hasattr(semiparametric, "k_medians_shm"):
                            learn_params["partition_fun"] = getattr(
                                semiparametric, "k_medians_shm"
                            )
...
def save_detector_assembly(
    assembled_detector: Dict[str, Any], filename: Optional[str] = None
) -> str:
    """
    ...
    config["training_function"] = (
        f"<function train_detector_{assembled_detector['suffix']}>"
    )
