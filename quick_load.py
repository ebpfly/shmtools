"""
Quick SHM Extension Loader - Just run this file!
"""

# Simple way to activate SHM extension
...
panel.style.cssText = `position:fixed; top:60px; right:20px; background:white; border:2px solid #007bff; 
    border-radius:8px; box-shadow:0 4px 20px rgba(0,0,0,0.15); z-index:1000; width:280px; 
    font-family:Arial,sans-serif;`;

// Header
const header = document.createElement('div');
header.innerHTML = '🔧 SHM Functions';
header.style.cssText = `background:#007bff; color:white; padding:12px 16px; font-weight:bold; 
    font-size:14px; border-radius:6px 6px 0 0; cursor:move;`;
...
Object.keys(funcs).forEach(category => {
    ...
    catHeader.style.cssText = `padding:10px 16px; background:#f8f9fa; font-weight:bold; 
        color:#495057; font-size:12px; border-bottom:1px solid #dee2e6;`;
    ...
    funcs[category].forEach(func => {
        ...
        item.style.cssText = `padding:10px 16px; cursor:pointer; border-bottom:1px solid #f1f3f4; 
...
print("🔧 SHM Extension loaded! Click functions to copy code to clipboard.")